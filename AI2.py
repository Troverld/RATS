...
class AI2:
    ...
    def partitioning_points(self, question: str, answer: str) -> List[Dict[str, str]]:
        """
        将 LaTeX 格式的答案分解为结构化要点
        
        参数:
            question: 输入的 LaTeX 格式问题
            answer: 需要分解的 LaTeX 格式答案
            
        返回:
            包含要点和解释的JSON结构
        """
        prompt = f"""
        请将以下 LaTeX 格式的答案按照逻辑切分，保持所有 LaTeX 表达式，以及解答的整体逻辑不变。
        ...
        输出必须为严格的 JSON 格式，包含 "key"（要点）和 "description"（解释）字段。它不能包含任何其他文本或格式化信息。
        ...
        {{
            "points": [
                {{
                    "key": "要点 1",
                    "description": "要点 1 对应的原文..."
                }},
                {{
                    "key": "要点 2",
                    "description": "要点 2 对应的原文..."
                }}
            ]
        }}
        """
        ...
        response = self.client.chat.completions.create(
            ...
            temperature = 0.2  # 保持低随机性以确保LaTeX完整性
        )
        ...
        # 解析并验证JSON输出
        try:
            resp = loads_preserve_backslashes_v2(response.choices[0].message.content.strip())
            ...
            return [[item['key'], item['description']] for item in result["points"]]
